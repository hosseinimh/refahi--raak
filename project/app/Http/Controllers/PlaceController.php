...
class PlaceController extends Controller
{
   ...
   public function rent_add(Request $request)
   {
      ...
      $place_rent = Place_Rent::where('place_id', $place_id)->first();
      // dd($place_info);
      if (!$place_info)
         return response()->json([
            '_result' => '0',
   ...
   public function rent_save(Request $request)
   {
      ...
      $frmData = [
         ...
         'docfile' => $request->uploadfile,


      ];
      if ($request->docfile) {
         $rand_postfix = rand(100, 999);
         $path = $request->docfile->store('temp/');
         Storage::move($path, "doc\\rent\\" . $place_id . "_rent_" . $rand_postfix . ".pdf");
         $frmData['docfile']  = $place_id . "_rent_" . $rand_postfix . ".pdf";
      }
   ...
   public function rent_del(Request $request)
   {

      $pid = $request->id;

      $crnplace = Place_Rent::where('id', $pid)->first();
      if ($crnplace) {
         $crnplace->delete();
   ...
   public function rent_update(Request $request)
   {
      ...
      $crnplace = Place_Rent::where('place_id', $place_id)->first();
